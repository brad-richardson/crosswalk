"""Dashboard routes for the crosswalk web UI."""
...
async def task_fetch(
    ...
    if dataset not in list_datasets():
        return HTMLResponse(status_code=404, content="Unknown dataset")
    cmd = ["uv", "run", "crosswalk", "data", "fetch", fetch_type, dataset]
...
async def task_features(
    ...
    if dataset not in list_datasets():
        return HTMLResponse(status_code=404, content="Unknown dataset")
    cmd = ["uv", "run", "crosswalk", "ml", "features", dataset]
    if force:
        cmd.append("--force")
...
async def task_backfill(
    ...
    cmd = ["uv", "run", "crosswalk", "labels", "backfill"]
    if not force:
