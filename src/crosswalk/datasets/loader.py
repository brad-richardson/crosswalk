...
Example usage:
    from crosswalk.datasets import DatasetLoader
...
class DatasetLoader:
    ...
    def load_reference(self, dataset_id: str) -> gpd.GeoDataFrame:
        ...
        if ref_path is None:
            ...
            raise FileNotFoundError(
                f"Reference file not found for '{dataset_id}'. Run: crosswalk fetch overture {base}"
            )
    ...
    def load_target(self, dataset_id: str) -> gpd.GeoDataFrame:
        ...
        if target_path is None:
            base = self._strip_osm_suffix(dataset_id)
            if dataset_id.endswith("_osm"):
                hint = f"Run: crosswalk fetch osm {base}"
            else:
                hint = f"Run: crosswalk fetch target {dataset_id}"
