...
def discover_classes(
    ...
    Examples:
        # Basic discovery
        crosswalk class discover data/raw/us_boston_streets.parquet
        ...
        crosswalk class discover data/raw/new_dataset.parquet --print-only
    """
...
def analyze_classes(
    ...
    Examples:
        ...
        crosswalk class analyze --from-labels

        # Full analysis with predictor on datasets with 50+ labels
        crosswalk class analyze --from-labels --train-predictor

        # Analyze from bridge file with predictor
        crosswalk class analyze bridge.parquet \\
            --reference ref.parquet --target target.parquet --train-predictor

        # Save detailed report
        crosswalk class analyze --from-labels --train-predictor --output analysis.json
    """
...
def detect_non_roads(
    ...
    Examples:
        ...
        crosswalk class detect-non-roads data/raw/br_sao_paulo_roads.parquet
...
def train_class_predictor(
    ...
    Examples:
        crosswalk class train-predictor bridge.parquet \\
...
def predict_classes(
    ...
    Examples:
        crosswalk class predict target.parquet
...
def update_class_mappings(
    ...
    Examples:
        ...
        crosswalk class update-mappings --dry-run

        # Update specific datasets (only those without existing class_mapping)
        crosswalk class update-mappings us_montana_helena us_usfs_lolo

        # Apply changes (no dry run)
        crosswalk class update-mappings us_montana_helena
    """
...
def suggest_class_mapping(
    ...
    Examples:
        ...
        crosswalk class suggest-mapping data/raw/new_city_roads.parquet

        # Specify different column names
        crosswalk class suggest-mapping data.parquet --class-column road_type

        # Save to JSON
        crosswalk class suggest-mapping data.parquet -o suggestions.json
    """
    import geopandas as gpd
