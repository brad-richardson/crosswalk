...
def register_commands(app: typer.Typer) -> None:
    ...
    def export_spark_model(
        ...
        Trains on config.SPARK_PORTABLE_FEATURES (35 of 83) and exports as
        XGBoost-native JSON loadable by the Spark MatchLayerToNetworkV2 job.

        Those 35 are a value-selected subset of the ~45 features computable from
        ...
        # Train with Spark-portable hyperparams (tuned for the 35-feature subset)
        matcher = MLMatcher()
        metrics = matcher.train(
            labels_dir=labels_dir,
            test_size=0.2,
