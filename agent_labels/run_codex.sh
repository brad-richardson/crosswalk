#!/bin/bash
# Run Codex on agent labeling batch
# Usage: ./run_codex.sh [batch_dir]
...
OUTPUT_DIR="$BATCH_DIR/labels/codex"
...
echo "=== Codex Run Started: $(date) ===" > "$LOG_FILE"
echo "" > "$RAW_OUTPUT"
...
LABEL = match/no_match/unsure, CONFIDENCE = 0.0-1.0, REASON = brief text (no commas)"
    ...
    # Run codex exec with output file
    timeout 30 codex exec -o "$TMPOUT" -- "$PROMPT" 2>&1 || true
    RAW=$(cat "$TMPOUT" 2>/dev/null)
