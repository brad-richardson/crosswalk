...
class TestRelationalFeatures:
    ...
    @pytest.mark.parametrize("line_b,expected_alignment", [
    ...
    def test_parallel_alignment(self, line_b, expected_alignment):
        ...
        assert compute_parallel_alignment(line_a, line_b) == pytest.approx(expected_alignment, abs=0.1)

    @pytest.mark.parametrize("sidewalk,expected_side", [
    ...
    def test_endpoint_proximity(self):
        ...
        start_prox, end_prox, shared = compute_endpoint_proximity(target, other_endpoints, tolerance=5.0)
...
class TestSpatialContextIndex:
    ...
    def test_endpoint_features(self, segments):
        ctx = SpatialContextIndex()
        ctx.build_from_gdf(segments, id_column="id")
        features = compute_endpoint_features(segments.iloc[0].geometry, ctx, exclude_segment_idx=0, tolerance=5.0)
