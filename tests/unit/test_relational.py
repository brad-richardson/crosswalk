...
class TestMultiPartGeometries:
    """Multi-part (MultiLineString) handling in the sibling search (#347).

    MultiLineStrings reach the sibling search from raw datasets that still
    contain them and from stored backfill geometries. They used to raise
    NotImplementedError at np.array(geom.coords), error-NaNing the WHOLE
    pair's feature set. They must instead be flattened (merge contiguous
    parts, else longest part) and produce real sibling features.
    """

    def test_flatten_contiguous_parts_merges(self):
        from shapely import MultiLineString

        from crosswalk.features.relational import flatten_multipart_line

        mls = MultiLineString([[(0, 0), (250, 0)], [(250, 0), (500, 0)]])
        flat = flatten_multipart_line(mls)
        assert flat.geom_type == "LineString"
        assert list(flat.coords) == [(0.0, 0.0), (250.0, 0.0), (500.0, 0.0)]

    def test_flatten_disjoint_parts_takes_longest(self):
        from shapely import MultiLineString

        from crosswalk.features.relational import flatten_multipart_line

        mls = MultiLineString([[(0, 0), (400, 0)], [(450, 0), (460, 0)]])
        flat = flatten_multipart_line(mls)
        assert flat.geom_type == "LineString"
        assert list(flat.coords) == [(0.0, 0.0), (400.0, 0.0)]
    ...
    def test_flatten_passthrough(self):
        ...
        assert flatten_multipart_line(line) is line
        empty = MultiLineString([])
        assert flatten_multipart_line(empty) is empty
        assert flatten_multipart_line(None) is None

    def test_multipart_segment_contiguous_matches_merged(self):
        """A contiguous multi-part segment behaves exactly like its merged line."""
        from shapely import MultiLineString, STRtree

        from crosswalk.features.relational import find_parallel_sibling

        eastbound_mls = MultiLineString([[(0, 0), (250, 0)], [(250, 0), (500, 0)]])
        eastbound_merged = LineString([(0, 0), (250, 0), (500, 0)])
        westbound = LineString([(0, 15), (500, 15)])

        spatial_index = STRtree([eastbound_merged, westbound])
        segment_data = [("eb", "I-90", "motorway"), ("wb", "I-90", "motorway")]

        result_mls = find_parallel_sibling(
            segment=eastbound_mls,
            segment_id="eb",
            segment_name="I-90",
            segment_class="motorway",
            spatial_index=spatial_index,
        ...
        result_merged = find_parallel_sibling(
            segment=eastbound_merged,
            segment_id="eb",
            segment_name="I-90",
            segment_class="motorway",
            spatial_index=spatial_index,
        ...
        assert result_mls == result_merged
        assert result_mls.has_sibling is True
        assert result_mls.sibling_distance == pytest.approx(15.0, abs=1.0)

    def test_multipart_segment_disjoint_uses_longest_part(self):
        """A disjoint multi-part segment behaves like its longest part."""
        from shapely import MultiLineString, STRtree

        from crosswalk.features.relational import find_parallel_sibling

        # Longest part spans x 0..400; a 10m fragment sits further along.
        eastbound_mls = MultiLineString([[(0, 0), (400, 0)], [(450, 0), (460, 0)]])
        longest_part = LineString([(0, 0), (400, 0)])
        westbound = LineString([(0, 15), (500, 15)])

        spatial_index = STRtree([longest_part, westbound])
        segment_data = [("eb", "I-90", "motorway"), ("wb", "I-90", "motorway")]
        ...
        result_longest = find_parallel_sibling(
            segment=longest_part,
            segment_id="eb",
            segment_name="I-90",
            segment_class="motorway",
            spatial_index=spatial_index,
        ...
        assert result_mls == result_longest
        assert result_mls.has_sibling is True

    @pytest.mark.parametrize(
        "candidate_parts",
        [
            [[(0, 15), (250, 15)], [(250, 15), (500, 15)]],  # contiguous
            [[(0, 15), (400, 15)], [(450, 15), (460, 15)]],  # disjoint
        ],
        ids=["contiguous", "disjoint"],
    )
    def test_multipart_candidate_context_and_fallback_parity(self, candidate_parts):
        """MLS candidates yield siblings, identically with and without context."""
        from shapely import MultiLineString, STRtree
        ...
        eastbound = LineString([(0, 0), (500, 0)])
        westbound_mls = MultiLineString(candidate_parts)

        geometries = [eastbound, westbound_mls]
        segment_ids = ["eb", "wb"]
        names = ["I-90", "I-90"]
        classes = ["motorway", "motorway"]
        spatial_index = STRtree(geometries)
        segment_data = list(zip(segment_ids, names, classes))

        context = build_sibling_search_context(geometries, segment_ids, names, classes)

        result_ctx = find_parallel_sibling(
            segment=eastbound,
            segment_id="eb",
            segment_name="I-90",
            segment_class="motorway",
            spatial_index=context.spatial_index,
            segment_data=context.segment_data,
            context=context,
        )
        result_fallback = find_parallel_sibling(
            segment=eastbound,
            segment_id="eb",
            segment_name="I-90",
            segment_class="motorway",
        ...
        assert result_ctx == result_fallback
        assert result_ctx.has_sibling is True
        assert result_ctx.sibling_distance == pytest.approx(15.0, abs=1.0)

    def test_context_precomputes_flattened_coords_for_multipart(self):
        """build_sibling_search_context stores flattened coords for MLS entries."""
        from shapely import MultiLineString

        from crosswalk.features.relational import build_sibling_search_context

        contiguous = MultiLineString([[(0, 15), (250, 15)], [(250, 15), (500, 15)]])
        disjoint = MultiLineString([[(0, 30), (400, 30)], [(450, 30), (460, 30)]])
        plain = LineString([(0, 0), (500, 0)])

        context = build_sibling_search_context(
            [plain, contiguous, disjoint],
            ["a", "b", "c"],
            [None, None, None],
            [None, None, None],
        )

        np.testing.assert_array_equal(context.segment_coords[0], np.array(plain.coords))
        np.testing.assert_array_equal(
            context.segment_coords[1],
            np.array([(0.0, 15.0), (250.0, 15.0), (500.0, 15.0)]),
        )
        np.testing.assert_array_equal(
            context.segment_coords[2], np.array([(0.0, 30.0), (400.0, 30.0)])
        )

    def test_pair_features_with_multipart_candidate_no_error_nan(self):
        """A pair whose sibling search hits an MLS candidate gets REAL features.

        Regression test for #347: the MLS candidate used to raise inside
        find_parallel_sibling, and the error path NaN'd the ENTIRE pair's
        feature set (silent signal loss).
        """
        from shapely import MultiLineString
        ...
        ref_geom = LineString([(0, 2), (500, 2)])
        target_geom = LineString([(0, 0), (500, 0)])
        # Multi-part twin of the target 15m away -> sibling candidate
        target_twin_mls = MultiLineString([[(0, 15), (250, 15)], [(250, 15), (500, 15)]])

        ref_context = build_sibling_search_context(
            [ref_geom], ["ref1"], ["Main Street"], ["residential"]
        )
        target_context = build_sibling_search_context(
            [target_geom, target_twin_mls],
            ["t1", "t2"],
        ...
        features = compute_pair_features(
            ...
            ref_seg_id="ref1",
            target_seg_id="t1",
            ref_sibling_context=ref_context,
            target_sibling_context=target_context,
        ...
        # No error path: error metadata keys are absent
        assert not any(k.startswith("_") for k in features), (
            f"Pair hit the error path: { {k: v for k, v in features.items() if k.startswith('_')} }"
        )
        # Both sibling searches ran: ref has no twin, target's twin is the MLS
        assert features["has_parallel_sibling_ref"] == 0.0
        assert features["likely_representation_mismatch"] == 1.0
        # Non-sibling features are real values, not error-NaNs
        assert features["hausdorff_distance_m"] == pytest.approx(2.0, abs=0.1)
