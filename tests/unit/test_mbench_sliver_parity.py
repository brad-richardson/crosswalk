"""Parity guard: mbench's standalone sliver rule must match matcher's.
...
# mbench lives in its own package tree with its own venv; add it to the path so
...
def test_thresholds_match():
    assert mbench_sliver.SLIVER_SPAN_THRESHOLD == matcher_config.SLIVER_SPAN_THRESHOLD
    assert mbench_sliver.SLIVER_ABS_OVERLAP_M == matcher_config.SLIVER_ABS_OVERLAP_M
...
def test_sliver_parity_over_grid(ref_span, tgt_span):
    for ref_len, tgt_len in product(_LENS, _LENS):
        ...
        actual = mbench_sliver.is_sliver_edge(ref_span, tgt_span, ref_len, tgt_len)
