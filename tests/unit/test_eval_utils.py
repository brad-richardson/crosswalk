...
from matcher.datasets.schema import DatasetConfig, QualityFingerprintConfig
from matcher.eval_utils import (
    TYPE_OVERRIDES,
    build_type_groups,
...
class TestClassifyDatasetTypeGroup:
    def test_road_good(self):
        ...
        with patch("matcher.eval_utils.get_dataset_config", return_value=config):
    ...
    def test_missing_config_returns_other(self):
        with patch("matcher.eval_utils.get_dataset_config", return_value=None):
...
class TestBuildTypeGroups:
    def test_mixed_datasets(self):
        ...
        with patch("matcher.eval_utils.get_dataset_config", side_effect=mock_get):
