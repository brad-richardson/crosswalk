"""Parity guard: cbench's set-label metric core must match matcher's.
...
``cbench.eval.stitch_metrics.set_label_metrics``. This test asserts the two agree
across a representative grid of predicted-edge / membership inputs, so they cannot
drift silently (mirrors ``test_cbench_sliver_parity.py``).
"""
...
# cbench lives in its own package tree; add it to the path so the main test suite
...
from cbench.eval.stitch_metrics import set_label_metrics as cbench_set_metrics  # noqa: E402
...
def test_set_metric_parity(pred, ref_members, tgt_members):
    ...
    actual = cbench_set_metrics(pred, ref_members, tgt_members)
